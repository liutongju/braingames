...
var vs_stimuli = [
  {set_size: 4,  target_present: true,   data: [{trialType: 'p04'}]},
  {set_size: 8,  target_present: true,   data: [{trialType: 'p08'}]},
  {set_size: 12, target_present: true,   data: [{trialType: 'p12'}]},
  {set_size: 4,  target_present: false,  data: [{trialType: 'a04'}]},
  {set_size: 8,  target_present: false,  data: [{trialType: 'a08'}]},
  {set_size: 12, target_present: false,  data: [{trialType: 'a12'}]}
];
var all_trials = jsPsych.randomization.repeat(vs_stimuli, 3);  // specify here with 2nd arg how many repetitions of each trial
...
var instructions_block = {
  ...
  pages: [   // "<p></p>",
    ...
    "<p>Press 'f' if you see the target stimulus or 'j' if you do not</p>" + "<br>" +
...
for(i = 0; i < all_trials.length; i += 1){
  ...
  timeline.push({
    ...
    target: "{{ gamestatic('img/target.png') }}",
    foil: "{{ gamestatic('img/distractor.png') }}",
    fixation_image: "{{ gamestatic('img/fixation.png') }}",
    timing_max_search: 5000,
    set_size: all_trials[i].set_size,
    target_present: all_trials[i].target_present,
...
var debrief_block = {
  type: "text",
  text: function() {
    return "<h1>Debriefing:</h1>" + "<p>The icons you searched for were edited carefully to determine if certain features make you slower or faster at finding it</p>" +
    "<br>" +
...
var images = ["{{ gamestatic('img/instAnswer.png') }}", "{{ gamestatic('img/keyhands.jpg') }}",       // instruction screen images
              "{{ gamestatic('img/target.png') }}", "{{ gamestatic('img/distractor.png') }}", "{{ gamestatic('img/fixation.png') }}"];
